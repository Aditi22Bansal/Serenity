...
export default function MoodCheckin() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSuccess(checkedIn ? 'Check-in updated! 🌟' : 'Check-in saved! 🌿');
      setCheckedIn(true);
      loadData();
  ...
  return (
    <div className="mood-page">
      <div className="container">
        ...
        {checkedIn && !success && <div className="mood-already"><p>✅ You've already checked in today! You can update it below.</p></div>}
        ...
        <div className="mood-card">
          ...
          <div className="mood-note">
            ...
            <textarea placeholder="How's your day going? What's on your mind..." value={form.note} onChange={e => setForm({ ...form, note: e.target.value })} maxLength={500} />
          </div>

          <div>
            <label style={{ display: 'block', fontWeight: 700, margin: '16px 0 10px', fontSize: '0.95rem' }}>🏷️ What did you do today?</label>
              ...
              {history.slice(0, 12).map((entry, i) => (
                <div className="mood-history-card" key={i}>
                  ...
                  <div className="mh-stats">
                    <span className="mh-stat" style={{ background: 'var(--peach-light)', color: 'var(--peach)' }}>⚡{entry.energy}</span>
                    <span className="mh-stat" style={{ background: 'var(--blue-light)', color: 'var(--blue)' }}>🧘{entry.stress}</span>
                    <span className="mh-stat" style={{ background: 'var(--lavender-light)', color: 'var(--lavender)' }}>💤{entry.sleep}</span>
                  </div>
                </div>
              ))}
            </div>
